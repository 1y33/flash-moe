"""Compare our FlashMoE kernel output against vLLM's Qwen3 MoE block.

Run:  .venv/bin/python -m benchmarks.test_correctness
"""
...
def main():
    ...
    our_out = flash_moe.forward(
        x.squeeze(0).float(), router_weight, gate_projs, up_projs, down_projs)
    ...
    for _ in range(warmup):
        flash_moe.forward(x_f32, router_weight, gate_projs, up_projs, down_projs)
    torch.cuda.synchronize()
